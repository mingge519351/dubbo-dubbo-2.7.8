...
public abstract class AbstractClusterInvoker<T> implements ClusterInvoker<T> {
    ...
    private Invoker<T> doSelect(LoadBalance loadbalance, Invocation invocation,
        ...
        if (invokers.size() == 1) {    // 1个不需要负载均衡
            return invokers.get(0);
        }
        Invoker<T> invoker = loadbalance.select(invokers, getUrl(), invocation); // AbstractLoadBalance
    ...
    public Result invoke(final Invocation invocation) throws RpcException {
        ...
        List<Invoker<T>> invokers = list(invocation); // 获取invoker
    ...
    protected LoadBalance initLoadBalance(List<Invoker<T>> invokers, Invocation invocation) {
        if (CollectionUtils.isNotEmpty(invokers)) {
            return ExtensionLoader.getExtensionLoader(LoadBalance.class).getExtension(invokers.get(0).getUrl()
                    .getMethodParameter(RpcUtils.getMethodName(invocation), LOADBALANCE_KEY, DEFAULT_LOADBALANCE)); // random 随机 负载均衡
        } else {
