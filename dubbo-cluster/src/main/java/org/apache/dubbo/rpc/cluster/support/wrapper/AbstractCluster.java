...
public abstract class AbstractCluster implements Cluster {
    ...
    public <T> Invoker<T> join(Directory<T> directory) throws RpcException {
        return buildClusterInterceptors(doJoin(directory), directory.getUrl().getParameter(REFERENCE_INTERCEPTOR_KEY)); // 构建链
    }
    ...
    protected class InterceptorInvokerNode<T> extends AbstractClusterInvoker<T> {
        ...
        public Result invoke(Invocation invocation) throws RpcException {
            Result asyncResult;
            try {
                interceptor.before(next, invocation); // 注册中心的拦截器操作
                asyncResult = interceptor.intercept(next, invocation); // ConsumerContextClusterInterceptor
            } catch (Exception e) {
                // onError callback
