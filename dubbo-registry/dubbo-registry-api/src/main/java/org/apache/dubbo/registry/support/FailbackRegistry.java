...
public abstract class FailbackRegistry extends AbstractRegistry {
    ...
    private void addFailedRegistered(URL url) {
        ...
        if (oldOne == null) {
            // never has a retry task. then start a new task for retry. // 时间轮重试
    ...
    protected void addFailedSubscribed(URL url, NotifyListener listener) {
        ...
        if (oldOne == null) {
            // never has a retry task. then start a new task for retry.
            retryTimer.newTimeout(newTask, retryPeriod, TimeUnit.MILLISECONDS); // 时间轮重试
        }
    }
    ...
    public void register(URL url) {
        ...
        try {
            ...
            addFailedRegistered(url); // 记录失败的url
        }
    }
    ...
    public void subscribe(URL url, NotifyListener listener) {
        ...
        try {
            ...
            // Record a failed registration request to a failed list, retry regularly
            addFailedSubscribed(url, listener); // 发生异常添加到错误订阅中
        }
    }
