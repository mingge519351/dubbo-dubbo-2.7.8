...
public class DubboProtocol extends AbstractProtocol {
    ...
    public <T> Exporter<T> export(Invoker<T> invoker) throws RpcException {
        ...
        exporterMap.put(key, exporter); // map存储，key：com.keming.demo.api.IDemoService:20880  value：dubboExpoter
    ...
    private void openServer(URL url) {
        ...
        if (isServer) {
            ...
            if (server == null) {
                ...
                // server supports reset, use together with override // 服务器已创建，根据url中的配置重置服务器
                server.reset(url);
            }
        }
    }
    ...
    public <T> Invoker<T> protocolBindingRefer(Class<T> serviceType, URL url) throws RpcException {
        ...
        // create rpc invoker.
        DubboInvoker<T> invoker = new DubboInvoker<T>(serviceType, url, getClients(url), invokers); // 创建dubboInvoker  getClients建立连接
    ...
    private ExchangeClient[] getClients(URL url) {
        ...
        int connections = url.getParameter(CONNECTIONS_KEY, 0); // 默认0
        List<ReferenceCountExchangeClient> shareClients = null;
        // if not configured, connection is shared, otherwise, one connection for one service
        if (connections == 0) {
            useShareConnect = true;  // 默认采用共享连接
            ...
            shareClients = getSharedClient(url, connections); // 获取共享连接
        }
    ...
    private List<ReferenceCountExchangeClient> getSharedClient(URL url, int connectNum) {
        ...
        synchronized (locks.get(key)) {
            ...
            connectNum = Math.max(connectNum, 1); // 默认1个连接
    ...
    private ExchangeClient initClient(URL url) {
        ...
        // client type setting.
        String str = url.getParameter(CLIENT_KEY, url.getParameter(SERVER_KEY, DEFAULT_REMOTING_CLIENT)); // 默认netty

        url = url.addParameter(CODEC_KEY, DubboCodec.NAME); // 编解码协议，默认dubbo
        // enable heartbeat by default
        ...
        try {
            ...
            if (url.getParameter(LAZY_CONNECT_KEY, false)) { // 默认新建连接
                client = new LazyConnectExchangeClient(url, requestHandler); // 延迟建立连接
